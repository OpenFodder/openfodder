...
function tilesForSeed(pSeed) {
    var ctx = MapGen.BuildCampaignPlan({
        Seed: pSeed, ProfileName: "ice_outpost",
    ...
    var counts = { 80: 0, 81: 0, 82: 0, 83: 0, 84: 0, 85: 0 };
    for(var x = 0; x < ctx.Width; ++x) {
        for(var y = 0; y < ctx.Height; ++y) {
            var t = MapGen.Layers.Get(ctx.RenderedMap.Tiles, x, y, 0);
...
var seeds = [1968570037, 12345, 99999, 50000, 777, 4242, 88888, 31337, 271828, 161803];
print("seed       80   81   82   83   84   85   river  lakes  ponds");
for(var si = 0; si < seeds.length; ++si) {
    var r = tilesForSeed(seeds[si]);
    var c = r.counts;
    var rivers = r.ctx.Rivers.length;
    var lakes = r.ctx.Lakes ? r.ctx.Lakes.length : 0;
    var ponds = r.ctx.Ponds.length;
    print(seeds[si] + "  " + c[80] + "    " + c[81] + "   " + c[82] + "    " + c[83] + "   " + c[84] + "    " + c[85] + "    " + rivers + "      " + lakes + "      " + ponds);
}
