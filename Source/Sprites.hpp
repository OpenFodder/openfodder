...
struct sSprite
{
	...
	int16 field_C;		// unused
	int16 field_E;		// unused
	...
	{
		int32 mFixedPoint;
		sSprite *mOwnerSprite; // Created by Sprite
	};
	...
	int8 field_44;
	int8 mActionCooldown;

	// Eventually this should be replaced
	union
	{
		int32 mHomePosPacked; // Enemy Helicopter home X/Y packed as (x << 16) | (y & 0xFFFF)
		sMission_Troop *mMissionTroop;
		sSprite *mSourceSprite;
	};

	int16 mWeaponFireTimer; // Weapon fire delay / projectile speed
	int8 mRetargetCooldown;
	int8 field_4D;		   // Unused
	int8 field_4E;		   // Unused
	int8 mInWater;		   // Player: -1 = In water
	int16 field_50;		   // Terrain/deviation flag (quicksand/water edge, projectile deviation, temp)
	int16 mRowsToSkip;	   // Video: Number of rows not to draw
	int8 mFiredWeaponType; // Human: Fired Weapon (1 = Grenade, 2 = Bullet, 3 = Rocket)
	int8 mWeaponAnimTick;  // Weapon fire animation counter / toggle
	int8 mDelayCounter;	   // Drop/launch delay counter (terrain drops, rocket/grenade warmup)
	int8 mWeaponCooldown;  // Human: Weapon Fired Cooldown
	int8 mHasShadow;	   // Has Shadow
	int8 mDirectionOverride;	// Direction/aim override active (skip re-aiming)
	...
	union
	{
		...
		size_t mSpriteScanIndex;		// Rolling index into mSprites for searches
		size_t mRescueTentIndex;		// Hostage rescue tent sprite index
		int32 mComputerAnimAccumulator; // Computer animation accumulator
	...
	int8 field_75;			// Power-up flags + heli call state (invincibility/homing, callpad)
