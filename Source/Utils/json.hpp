...
/* Since _snprintf has security problems on VS2013 and not necessarily to define _CRT_SECURE_NO_WARNINGS */
	...
	private:
			...
			const unsigned int currentIndent = 0) const NOEXCEPT
		{
			...
			case (value_t::number_float) :
			{
				...
				const auto sz = static_cast<unsigned int>(c99_snprintf(nullptr, 0, "%.15g", m_value.number_float));
				std::vector<typename string_t::value_type> buf(sz + 1);
				c99_snprintf(&buf[0], buf.size(), "%.15g", m_value.number_float);
#endif
		...
		public:
			...
			*/
				...
				inline void yyfill() NOEXCEPT
			{
				...
				const size_t offset_start = m_start - m_content;
				const size_t offset_marker = m_marker - m_start;
				const size_t offset_cursor = m_cursor - m_start;
