...
int16 cFodder::Mission_Loop( cSurface* pImage ) {
	...
	//pImage->load( mSurfaceMapOverview );
	//pImage->Save();
	mSurfacePlayfield->paletteSet( mPalette, 0, true );
	g_Window.RenderAt( mSurfacePlayfield, cPosition() );
	g_Window.FrameEnd();
...
void cFodder::Mission_Sprites_Handle( cSurface* pImage ) {
...
void cFodder::sub_124DB() {
	if (dword_3B48B[0] == (sSprite_0*) -1 || dword_3B48B[0] == 0)
		return;
...
void cFodder::sub_12A5F() {
	...
	for (int16 Data0 = 7; Data0 >= 0; --Data0, ++Data20) {
		if (Data20->field_4 == (sSprite_0*) -1 || Data20->field_4 == 0 )
			continue;
...
void cFodder::Recruit_Show() {
	...
	cSurface* ImageHill = new cSurface(352,230);
...
void cFodder::sub_2C7E1( cSurface* pImage ) {
...
void cFodder::sub_2CA3F( cSurface* pImage ) {
	...
	map_Tiles_Draw_();
	pImage->CopyFrom( mSurfacePlayfield,  word_40056 );
}
...
void cFodder::sub_2CDC0( cSurface* pImage ) {
	int16 word_39707 = 0;
	int16 word_39709 = 0;
...
void cFodder::sub_2EACA( cSurface *pImage ) {
	word_3AC45 = -1;
	sub_30465();
...
int16 cFodder::introPlayText() {
	...
	for ( word_3B2CF = 0; mIntroText[word_3B2CF].mImageNumber != 0; ++word_3B2CF) {
		...
		if (mIntroText[word_3B2CF].mImageNumber != 0xFF) {
			...
			CurrentImage = new cSurface( 352, 230 );
			CurrentImage->paletteSet( mPalette, 0xD0 );
			word_3B447 = 0xAF;
...
int16 cFodder::sub_20A39( sSprite_0* pSprite ) {
...
void cFodder::map_Tiles_Draw() {
	delete mSurfacePlayfield;
	mSurfacePlayfield =  new cSurface( (readLEWord( &mMap[0x54] ) + 1) * 16, (readLEWord( &mMap[0x56] ) + 1)* 16 );
	mSurfacePlayfield->paletteSet( mPalette, 0, true );
	...
	for (uint16 cx = 0; cx < readLEWord( &mMap[0x56] ); ++cx) {

		uint8* si2 = si;

		for (uint16 cx2 = 0; cx2 <= readLEWord( &mMap[0x54] ); ++cx2) {

			video_Draw_MapTile( mSurfacePlayfield, readLEWord(si2), cx2, cx );
...
void cFodder::sub_2F9B3(cSurface* pImage) {
...
void cFodder::sub_30E49() {
	...
	for (; Data1C >= 0; --Data1C, ++Data24 ) {
		...
		if ( Data24->field_4 == (sSprite_0*) -1 || Data24->field_4 == 0 )
			continue;
...
int16 cFodder::sub_305D5( sSprite_0*& pData20 ) {
	...
	if (*Data30 == (sSprite_0*) -1 || *Data30 == 0)
		goto loc_306AD;
...
loc_306BE:;
	...
	word_39FCE = pData20;
	if (pData20 == (sSprite_0*) -1 || pData20 == 0 )
		return -1;
...
void cFodder::sub_31075( int16 pData0 ) {
	...
	if (sub_305D5( Data20 ) >= 0) {
		word_3A067 = byte_3A05A[0];
		word_3A069 = byte_3A05A[4];
		word_3AA1B = 1;
		word_3A054 = 0;
	}
